...
`process_redfin_market_data.py` is separate from that chain — run it after `process_realtor_data.py` (it derives its zip list from `scripts/seed_data.json` plus the three hand-seeded zips in `seed.sql`) and before a reset:

```bash
python scripts/process_redfin_market_data.py   # -> supabase/seed_market_trends.sql
```

It streams the Redfin Data Center's zip-code tracker (~1.4GB gzipped) and filters in flight, so nothing large hits disk — but it does download the whole file, so expect it to take a while. It keeps only the `All Residential` aggregate (the file also carries Condo/Townhouse/Single-Family/Multi-Family series per zip-month, which would quintuple the rows for nothing) for Texas zips that appear in the listings. Currently 6,978 zip-month observations across all 41 seeded zips, 2012-01 to 2026-03, with **zero** properties lacking coverage. Redfin refreshes on the third Friday of each month; re-running is how you update. The script hard-fails if the upstream column order changes rather than silently writing garbage into the wrong columns.
...
`enable_confirmations = false` in `supabase/config.toml`, so `signUp` returns a live session and the user lands signed in. **Turning it on is now safe** — set it under `[auth.email]` (not the `[auth.sms]` block, which has an identically named key) and restart with `npx supabase stop && npx supabase start`; a config change alone won't take. With it on, `signUpAction` returns `status: 'confirm-email'` and `AuthForm` swaps the form for a "check your inbox" screen. That's modelled as its own status rather than an error — an early version rendered it in the red danger box, which reads as a failure when it is a success.

`app/auth/confirm/route.ts` is where the emailed links land. It handles **two** shapes, because which one arrives depends on the email template, not on this codebase:

- `?token_hash=...&type=signup` — what `supabase/templates/confirmation.html` sends, verified with `verifyOtp`.
- `?code=...` — what Supabase's *default* template produces, since `@supabase/ssr` forces `flowType: 'pkce'` and its hosted verify endpoint redirects back with a PKCE code. Exchanged with `exchangeCodeForSession`.

Handle only one and a template change silently breaks signup, surfacing as a dead link in a real user's inbox rather than anywhere a developer would notice. The custom template exists because the default routes through Supabase's hosted endpoint, which on some flows returns the session in a **URL hash fragment** — never sent to the server, so an App Router route handler cannot read it.

Failures redirect to `/login?error=...` and `LoginPage` threads that into `AuthForm` as `initialError`, so an expired or already-used link says which. `emailRedirectTo` must be absolute, so `next` arrives fully qualified and `resolveNext()` reduces same-origin URLs back to a path before `safeRedirectPath` judges them — an offsite `next` is dropped rather than followed.

Two URL settings in `config.toml` were wrong out of the box and are now fixed: `site_url` pointed at port **3000** (this app serves on **3050**) and `additional_redirect_urls` was **https**://127.0.0.1:3000. Both are baked into emailed links, so every confirmation link would have 404'd or been rejected as an unpermitted redirect. Local mail is caught by Mailpit at **http://127.0.0.1:54324** (API at `/api/v1/messages`), which is how to read a confirmation link without a real mailbox.
...
**Area Market Trends** on the property detail page (`components/property/MarketTrends.tsx`) covers the spec line about showing the price "with the option to view the area's historical pricing data or market trends" — collapsed by default to honour *option to view*, though the data is already server-rendered so expanding is free.

It is **area-level, deliberately**. The listings come from a Kaggle export with no real transaction record, so a per-property price history would have to be invented; the section is keyed on `properties.zip_code`, and both the subheading and the footnote say outright that it describes the ZIP and not the home. Keep that framing if you touch it — it's the whole reason this was buildable honestly.

Worth knowing about the sourcing, because the product docs are misleading here: **the Zillow, Realtor.com and Redfin APIs named in `files/info.md` are not obtainable.** Zillow's public API was retired in 2021 and its replacement (Bridge Interactive) is MLS/broker-only at ~$500/mo; Redfin has no API at all. What Redfin *does* publish is the free Data Center — aggregated market-trend TSVs on S3, no key, no signup. That's the source here. Third-party "Zillow APIs" that turn up in search results are scrapers, not Zillow. Note also that the sources' own MoSCoW list puts "full-scale integration with third-party services for property listings" in **Won't Have**, so the narrative and the prioritization disagree; the prioritization is the one that defines the MVP.

**Price prediction** (`lib/market/forecast.ts`) sits in the same section and covers the Should Have "AI-driven price prediction and trend analysis". The split matters: **the numbers are plain statistics, the AI only narrates them.** Asking a model to predict a price gets you a confident figure with no derivation; `forecastPrices()` is log-linear OLS over a trailing 60 months, and `lib/ai/market-forecast.ts` is handed the result to interpret.

Three decisions worth not undoing:

- **The projection anchors on a trailing 3-month mean, not on the fitted line.** Reading the projection off the regression 12 months past its end produces jumps when recent prices sit off the line — ZIP 78701 had a −2.1%/yr slope but a fitted value 21% *above* the latest actual, which is indefensible to a user however sound the arithmetic. Anchoring costs a small quantified bias (a 3-month mean lags a compounding series by ~1 month, −0.4% at 5%/yr growth) and repaid it several times over: measured 12-month error on the seeded ZIPs fell from 10.6/10.8/16.6% to **7.2/8.9/11.5%**.
- **The uncertainty band is measured, not assumed.** Textbook OLS prediction intervals assume independent residuals; monthly medians are strongly autocorrelated, so those intervals come out far too narrow. `backtest()` walks the series forward re-fitting at each origin, and the band is the 10th–90th percentile of real 12-month errors. It runs the *same* procedure the live projection uses, so the number describes what actually ships.
- **r² is usually terrible here (0.01–0.20)** because five years spanning the 2021 spike and the plateau after it genuinely have no clean log-linear trend. That's not a bug; below 0.25 the UI says outright that the projection is barely better than assuming flat.
...
- **Seed order is load-bearing.** `sql_paths` runs `seed.sql` → `seed_realtor.sql` → `seed_vectors.sql` → `seed_market_trends.sql`, and each of the first three depends on the previous. (`seed_market_trends.sql` is keyed on zip code, not property id, so it's order-independent — it's last only because it was added last.) `seed.sql` inserts ids 1-3 explicitly, which does *not* advance the `GENERATED BY DEFAULT AS IDENTITY` sequence, so it ends with a `setval` — without that, `seed_realtor.sql` (which omits ids) restarts at 1 and dies on the primary key. Realtor rows therefore occupy ids 4-103, and `seed_vectors.sql` hardcodes those absolute ids. Reordering the list, or inserting ids in `seed.sql` without updating the vector ids, silently misaligns embeddings against listings.
