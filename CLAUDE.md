...
- `app/dashboard/offers/page.tsx` (`SellerOffersInbox`) is the seller's incoming-offers inbox, filtered via `properties!inner` + `.eq('properties.owner_id', ...)` — offers RLS alone returns both sides (buyer's own offers and offers on owned listings mixed together), so the inner-join filter is what narrows it to just the seller side. `DashboardView`'s `MyOffersSection` is the buyer-side counterpart, showing "Pay Earnest Money" once accepted and unpaid, or "Check your email to sign" while a DocuSign envelope is out but unpaid.
- There's no Realtime subscription anywhere — a buyer/seller needs a fresh page load (Server Actions call `revalidatePath`) to see a status change; `OfferWizard`'s post-submit "Offer Sent" screen is a one-time snapshot of the initial submit response, not live, and intentionally isn't where accept/reject/pay live.
